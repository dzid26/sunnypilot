...
class TestSunnylinkdMethods:
  ...
  def setup(self, mocker):
    ...
    mocker.patch.object(sunnylinkd, 'save_param_from_base64_encoded_string', mock_save_param)  # ty: ignore[invalid-assignment]

    # Mock params with IsEngaged=False by default
    self.mock_params = mocker.MagicMock()
    self.mock_params.get_bool.return_value = False
    mocker.patch.object(sunnylinkd, 'params', self.mock_params)  # ty: ignore[invalid-assignment]
